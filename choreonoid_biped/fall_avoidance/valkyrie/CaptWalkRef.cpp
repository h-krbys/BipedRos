...
public:
  ...
  virtual bool control() override
  {
    ...
    case DSP:
      ...
      printf("step %d / %d\n", count, (int)footstep.size() - 1 );
      if(count == (int)footstep.size() - 1) {
        phase = STOP;
        break;
      }
      ...
      input.cop = footstep[count].cop;
      input.icp = footstep[count].icp;

      swing->set(input.swf, footstep[count + 1].pos, 0);
      input.elapsed  = 0;
      input.duration = swing->getTime();
      input.land     = footstep[count + 1].pos;

      trajectory->set(input, supportFoot);
      copRef = trajectory->getCop(0);
      icpRef = trajectory->getIcp(0);
